...
def reset_database():
    ...
    try:
        ...
        with open('Notes_Schema.sql', 'r', encoding='utf-8') as f:
            sql_script = f.read()
        ...
        # Verify data
        print("\n--- Verifying data ---")
        cursor.execute("SELECT COUNT(*) FROM dbo.Notes")
        notes_count = cursor.fetchone()[0] # type: ignore
        print(f"Notes: {notes_count}")
        
        cursor.execute("SELECT COUNT(*) FROM dbo.Tags")
        tags_count = cursor.fetchone()[0] # type: ignore
        print(f"Tags: {tags_count}")
        
        cursor.execute("SELECT COUNT(*) FROM dbo.NoteTags")
        note_tags_count = cursor.fetchone()[0] # type: ignore
        print(f"Note-Tag assignments: {note_tags_count}")
        
        if notes_count > 0:
            cursor.execute("SELECT Title FROM dbo.Notes ORDER BY CreatedAt DESC")
            print("\nNotes:")
