...
def reset_database():
    ...
    try:
        ...
        with open('Movies_Schema.sql', 'r', encoding='utf-8') as f:
            sql_script = f.read()
        ...
        print("Sample movies and ratings added")
        
        # Verify data
        print("\n--- Verifying data in database ---")
        cursor.execute("SELECT COUNT(*) FROM dbo.Movies")
        movies_count = cursor.fetchone()[0]# type: ignore
        print(f"Movies in database: {movies_count}")
        
        cursor.execute("SELECT COUNT(*) FROM dbo.Ratings")
        ratings_count = cursor.fetchone()[0] # type: ignore
        print(f"Ratings in database: {ratings_count}")
        
        if movies_count > 0:
            cursor.execute("SELECT TOP 5 Title, [Year], AvgScore, Votes FROM dbo.vMoviesRanking ORDER BY AvgScore DESC")
            print("\nTop Movies:")
            for row in cursor.fetchall():
                avg = row[2] if row[2] else 0.0
                votes = row[3] if row[3] else 0
                print(f"  - {row[0]} ({row[1]}) - {avg:.2f} ({votes} votes)")
